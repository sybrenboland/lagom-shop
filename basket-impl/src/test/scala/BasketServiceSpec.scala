...
class BasketServiceSpec extends AsyncWordSpec with Matchers with BeforeAndAfterAll with ExtraTransportExceptions {
  ...
  "Basket Service" should {
    ...
    "remove one item and get the basket" in {
      client.addItem("basketRemove").invoke(Item("Apple", 50)).flatMap { response =>
        response should ===(NotUsed)

        client.removeItem("basketRemove").invoke(Item("Apple", 50)).flatMap { response =>
          response should ===(NotUsed)

          client.getBasket("basketRemove").invoke().map { getItemsResponse =>
            getItemsResponse should ===(Basket(Seq(), 0))
          }
        }
      }
    ...
    "return an error if an item is removed after the order is placed" in {
      recoverToSucceededIf[BadRequest] {
        for (a <- client.addItem("basketRemove").invoke(Item("Apple", 50));
             b <- client.placeOrder("basketRemove").invoke();
             c <- client.removeItem("basketRemove").invoke(Item("Apple", 50)))
          yield c should ===(NotUsed)
      }
    }
    ...
    "return an error if an item is removed but is not in the basket" in {
      recoverToSucceededIf[BadRequest] {
        for (a <- client.removeItem("basketRemoveError").invoke(Item("Apple", 50)))
