...
class BasketSpec extends WordSpecLike with Matchers with BeforeAndAfterAll with TypeCheckedTripleEquals {
  ...
  "Basket" must {
    ...
    "Return InvalidCommand if remove item is called after the order is placed" in {
      val driver = new PersistentEntityTestDriver(system, new BasketEntity, "BasketRemove")
      driver.run(AddItem(Item("Apple", 50)))
      driver.run(PlaceOrder)
    ...
    "Return InvalidCommand if remove item is called but item is not in basket" in {
      val driver = new PersistentEntityTestDriver(system, new BasketEntity, "BasketRemove")
      driver.run(AddItem(Item("Apple", 50)))

      val outcome = driver.run(RemoveItem(Item("Orange", 40)))
      outcome.replies should ===(List(InvalidCommandException("The basket does not contain this item")))
    }
